  ...
  <div style="width: 220px;height: 400px;border:solid 1px;background-color: #222d32;overflow-y: auto;">
    <ul class="aside-nav">
      <li v-for="(item1,index) in navData" :key="'lay1'+index">
        <a :class="{'active':activeIndex_1===item1.id, 'open': openIndex_1===item1.id}" @click="clkItem([item1])">
          <i class="i-l">&nbsp;</i>
          <span>{{item1.name}}</span>
          <i class="i-r arrow" v-if="item1.children&&item1.children.length>0"><</i>
        </a>
        <ul :style="{maxHeight:openIndex_1===item1.id?item1.children.length*34+'px':'0px'}">
          <li v-for="(item2,index) in item1.children" :key="'lay2'+index">
            <a :class="{'active':activeIndex_2===item2.id}" @click="clkItem([item1,item2])">
              <i class="i-l">&nbsp;</i>
  ...
  methods: {
    clkItem: function (arr) {
      ...
      if (current.url) {
        ...
        // step2 - 激活当前
        this.activeIndex_2 = current.id;
        // step3 - 激活最上层
        this.activeIndex_1 = first.id;
      } else if (current.children && current.children.length > 0) {
        // 开启子节点
        this.openIndex_1 = this.openIndex_1 === current.id ? '' : current.id;
        this.activeIndex_2 = '';
      }
    },
    clkItem_bf: function (info) {
